 ...
 * support pagination, so this call retrieves all teams in one request.
 *
...
async function syncGames(options = {}) {
  ...
  while (pageCount < maxPages) {
    ...
    // no season is provided, the API returns games across all seasons.
    const params = { per_page };
...
async function syncStats(options = {}) {
  ...
  while (pageCount < maxPages) {
    ...
    // BUG: The following redeclaration of `cursor` causes a ReferenceError.
    // It shadows the outer cursor and makes earlier references invalid.
