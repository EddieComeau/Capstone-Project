...
// Derived metrics, cursor persistence, and merge of BDL vs computed sources.
...
async function syncAdvancedStatsEndpoint(type, options = {}) {
  ...
  // For simplicity, if a player list was fetched, we re-merge all players for season/scope (could be optimized)
...
/**
 * computeAdvancedStats: compute sums & averages from Stat docs and write to sources.computed,
 * then compute specific metrics and merge.
 */
async function computeAdvancedStats() {
  ...
  const playerAgg = Stat.aggregate([
    ...
    { $group: { _id: { playerId: '$playerId', season: '$season' }, count: { $sum: 1 }, statsList: { $push: '$stats' } } }
  ...
  for await (const g of playerAgg) {
    ...
    await AdvancedMetric.updateOne(
      filter,
      { $set: { 'sources.computed': computedObj, raw: { computed: computedObj }, updatedAt: new Date() }, $setOnInsert: { entityType: 'player', entityId: playerId, season, scope } },
      { upsert: true }
    );
  ...
  for await (const g of teamAgg) {
    ...
    await AdvancedMetric.updateOne(
      filter,
      { $set: { 'sources.computed': computedObj, raw: { computed: computedObj }, updatedAt: new Date() }, $setOnInsert: { entityType: 'team', entityId: teamId, season, scope } },
      { upsert: true }
    );
...
/**
 * computeSpecificMetricsForPlayerSeason: compute specific metrics and save them inside sources.computed.specific
 * Implements:
 * - passer_rating (NFL formula)
 * - completion_pct
 * - yards_per_attempt
 * - yards_per_target (if targets/receptions exist)
 * - yards_per_carry (for rushers)
 */
async function computeSpecificMetricsForPlayerSeason(playerId, season, scope = 'season') {
  ...
  // Each component bounded 0..2.375; passer_rating = ((a+b+c+d)/6)*100
  ...
  await AdvancedMetric.updateOne(
    filter,
    { $set: { 'sources.computed.specific': specific, updatedAt: new Date() } },
    { upsert: true }
  );
...
/* For teams we can compute team-specific specific metrics similarly */
...
async function mergeAdvancedSources(entityType, entityId, season, scope = 'season') {
  ...
  // bdl may have nested metrics, try to extract a normalized object
  ...
  // If computed.specific exists and bdl lacks certain keys, copy them
  ...
  // Also if bdl provides specific sub-object, keep it under metadata, merged already prefers bdl
...
/* Reuse earlier computeStandings and computeMatchups (omitted here to save space) */
async function computeStandings() {
  // same implementation as earlier (aggregate Game docs to Standing)
  // For brevity, call the previous implementation if exists; else include code
  ...
  for (const season of seasons) {
    ...
    for (const g of games) {
      ...
      const homeId = g.home_team?.id, visitorId = g.visitor_team?.id;
      ...
      byTeam[homeId].PF += Number(homeScore); byTeam[homeId].PA += Number(visitorScore); byTeam[homeId].games++;
      byTeam[visitorId].PF += Number(visitorScore); byTeam[visitorId].PA += Number(homeScore); byTeam[visitorId].games++;
      if (homeScore > visitorScore) { byTeam[homeId].wins++; byTeam[visitorId].losses++; }
      else if (homeScore < visitorScore) { byTeam[visitorId].wins++; byTeam[homeId].losses++; }
      else { byTeam[homeId].ties++; byTeam[visitorId].ties++; }
    }
    const ops = [];
    for (const [teamIdStr, data] of Object.entries(byTeam)) {
      ...
      ops.push({ updateOne: { filter: { teamId, season }, update: { $set: { teamId, season, wins, losses, ties, pointsFor: PF, pointsAgainst: PA, winPct, raw: data, updatedAt: new Date() } }, upsert: true } });
      if (ops.length >= BULK_BATCH_SIZE) { await Standing.bulkWrite(ops.splice(0, ops.length), { ordered: false }); }
    }
...
async function computeMatchups() {
  ...
  for await (const g of gamesCursor) {
    ...
    const week = g.week;
    const homeTeamId = g.home_team?.id, visitorTeamId = g.visitor_team?.id;
    ...
    for (const key of keysToCompare) {
      const h = Number(homeMetricsObj[key] || 0); const v = Number(visitorMetricsObj[key] || 0);
      comparison[key] = { home: h, visitor: v, diff: Number((h-v).toFixed(3)) };
    }
    await Matchup.updateOne({ gameId }, { $set: { gameId, season, week, homeTeamId, visitorTeamId, homeMetrics:homeMetricsObj, visitorMetrics:visitorMetricsObj, comparison, raw:g, updatedAt: new Date() } }, { upsert: true });
  }
}

/* -------------------- Injuries: same as earlier -------------------- */
async function syncInjuriesFromAPI(options = {}) {
  // identical to previous implementation, uses ballDontLieService.listPlayerInjuries in a cursor loop,
  // persists cursor if options.persist true under key 'injuries'.
  const per_page = Number(options.per_page || 100);
  const maxPages = Number(options.maxPages || 1000);
  const persist = options.persist !== false;
  const resume = options.resume !== false;
  const key = 'player_injuries'; // global key
  let cursor = resume ? await getSyncCursor(key) : options.cursor || null;
  let pageCount = 0, fetched = 0, upsertCount=0, previousCursor=null;
  ...
  while (pageCount < maxPages) {
    ...
    fetched += items.length;
    if (cursor && cursor === previousCursor) { console.warn('Cursor did not advance; stopping'); break; }

    for (const rec of items) {
      if (!rec || !rec.player) continue;
      const player = rec.player, team = player.team || {};
      const playerId = player.id || null, teamId = team.id || null;
      const status = rec.status || 'Unknown', description = rec.comment || '';
      ...
      const updateDoc = { playerId, teamId, status, description, source:'balldontlie', reportedAt, raw:rec, updatedAt:new Date() };
      try { await Injury.updateOne(upsertFilter, { $set: updateDoc }, { upsert:true }); upsertCount++; } catch(err) { console.warn('Failed injury upsert', err && err.message ? err.message : err); }
    }
